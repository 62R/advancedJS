Vue.component('cart', {
    ...
    methods: {
        addProduct(product) {
            ...
            //     }
            // });
            //$parent.postJson(this.cartUrl, product);
            console.log('addProduct done!');
        },
        remove(cartItem) {
            cart.list.splice(cart.list.indexOf(cartItem), 1);
        },
    ...
    <div class="cart-popup">
        ...
        <ul class="cart-popup__list" v-show="cartShow">
            ...
            <cart-popup-item v-for="item of cart.list" :cartItem = "item" :key="item.id"></cart-popup-item>
        </ul>    
    </div>
    `
});
        ...
        <li class="cart-popup-item">
            ...
            <div class="cart-popup-item__content">
                ...
                <div class="cart-popup-item__quantity-wrap">
                    ...
                    <button @click="reduceQuantity(cartItem)" class="cart-popup-item__quantity-btn"><i class="fa-regular fa-circle-minus"></i></button>
                    <button @click="increaseQuantity(cartItem)" class="cart-popup-item__quantity-btn"><i class="fa-regular fa-circle-plus"></i></button>            
                </div>
                <p class="cart-popup-item__total">{{cartItem.price*cartItem.quantity}}</p>
                <p class="cart-popup-item__price">{{cartItem.price}}</p>
                <button @click="remove(cartItem)" class="cart-popup-item__remove-btn"><i class="fa-solid fa-trash"></i></button>            
            </div>            
        </li>
    `
})